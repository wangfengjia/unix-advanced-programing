...
/**
 * Internal index file constants. There are used to construct records in the index file and data file
 */
...
/**
 * Library's private representation of database
 */
typedef struct {
    int idxfd;
    int datfd;
     ...
     */
      ...
      */
      if ((db->idxbuf = malloc(IDXLEN_MAX + 2)) == NULL){
          err_dump("_db_alloc: malloc error for idx buffer");
      }
...
/**
 * Free up a DB structure, and all the malloc'ed buffers it may point to. Also close the file descriptors if still open
 * @param db
 */
static void
_db_free(DB *db){
...
_db_readidx(DB *db, off_t offset){
    ...
    if ((i = read(db->idxfd, &iov[0], 2)) != PTR_SZ + IDXLEN_SZ){
        if (i == 0 && offset == 0){
    ...
    if (db->idxbuf[db->idxlen - 1] != NEWLINE){
        err_dump("_db_readidx: missing new line");
    }
    db->idxbuf[db->idxlen - 1] = 0; // replace new line with null

    //find the separators in index record
...
_db_writedat(DB *db, const char *data, off_t offset, int whence){
    ...
    /**
     * If we're appending, we have to lock before doing the lseek and write to make the two an atomic operation.
     * If we're overwriting an existing record, we don't have to lock
     */
     ...
     iov[0].iov_len = db->datlen - 1; //exclude newline
...
_db_writeidx(DB *db, const char *key, off_t offset, int whence, off_t ptrval){
    ...
    sprintf(db->idxbuf, "%s%c%lld%c%ld\n", key, SEP, (long long)db->idxbuf, SEP, (long)db->datlen);
...
_db_writeptr(DB *db, off_t offset, off_t ptrval){
    ...
    if (write(db->idxfd, asciiptr, PTR_SZ) != PTR_SZ){
        err_dump("_db_writeidx: write error of ptr field");
    }
}
...
_db_findfree(DB *db, int keylen, int datlen){
    ...
    if (writew_lock(db->idxfd, FREE_OFF, SEEK_SET, 1) < 0){
        err_dump("_db_findfrss: writew_lock error");
    }
...
_db_dodelete(DB *db){
    ...
    *ptr = 0; //null terminate for _db_writedat
    ...
    //lock the free list
    if (writew_lock(db->idxfd, FREE_OFF, SEEK_SET, 1) < 0){
        err_dump("_db_delete: writew_lock error");
    }

    //write data record with all blanks
...
_db_find_and_lock(DB *db, const char *key, int writelock){
    ...
    //Lock the hash chain here, The caller must unlock it when done. Note we lock and unlock only the first byte
    if (writelock){
    ...
    //Get the offset in the index file of first record on the hash chain (can be 0)
    ...
    while (offset != 0){
        ...
        offset = nextoffset; //next to compare
    }
...
//open or creata a databse. same arguments as open(2)
DBHANDLE
db_open(const char *pathname, int oflag, ...){
    ...
    db->nhash = NHASH_DEF; //hashtable size
...
void
db_close(DBHANDLE db){

    _db_free((DB *)db); //close fds, free buffers & struct
}
...
db_store(DBHANDLE h, const char *key, const char *data, int flag){
    ...
    if (_db_find_and_lock(db, key, 1) < 0){ //record not found
        ...
        if (flag == DB_REPLACE){
            rc = -1;
            db->cnt_storerr++;
            errno = ENOENT; //record does not exist
            goto doreturn;
        }
        /*
        ...
        */
        ...
        if (datlen != db->datlen){
            ...
            //new records goes to front of the hash chain
            _db_writeptr(db, db->chainoff, db->idxoff);
            db->cnt_stor3++;
        } else{
